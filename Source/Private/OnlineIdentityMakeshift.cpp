...
bool FUserOnlineAccountMakeshift::GetAuthAttribute(const FString& AttrName, FString& OutAttrValue) const
{
...
bool FUserOnlineAccountMakeshift::GetUserAttribute(const FString& AttrName, FString& OutAttrValue) const
{
...
inline FString GenerateRandomUserId(int32 LocalUserNum)
{
	...
	const bool bForceUniqueId = FParse::Param( FCommandLine::Get(), TEXT( "StableMakeshiftID" ) );
...
bool FOnlineIdentityMakeshift::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	FString ErrorStr;
	TSharedPtr<FUserOnlineAccountMakeshift> UserAccountPtr;
	...
	else
	{
		...
		if (UserId == NULL)
		{
			...
			UserAccountPtr = MakeShareable(new FUserOnlineAccountMakeshift(RandomUserId));
		...
		else
		{
			...
			TSharedRef<FUserOnlineAccountMakeshift>* TempPtr = UserAccounts.Find(*UniqueIdStr);
			check(TempPtr);
...
bool FOnlineIdentityMakeshift::Logout(int32 LocalUserNum)
{
...
bool FOnlineIdentityMakeshift::AutoLogin(int32 LocalUserNum)
{
	FString LoginStr;
	FString PasswordStr;
	FString TypeStr;
...
TSharedPtr<FUserOnlineAccount> FOnlineIdentityMakeshift::GetUserAccount(const FUniqueNetId& UserId) const
{
	TSharedPtr<FUserOnlineAccount> Result;

	FUniqueNetIdString StringUserId(UserId);
	const TSharedRef<FUserOnlineAccountMakeshift>* FoundUserAccount = UserAccounts.Find(StringUserId);
...
TArray<TSharedPtr<FUserOnlineAccount> > FOnlineIdentityMakeshift::GetAllUserAccounts() const
{
	TArray<TSharedPtr<FUserOnlineAccount> > Result;
	
	for (TMap<FUniqueNetIdString, TSharedRef<FUserOnlineAccountMakeshift>>::TConstIterator It(UserAccounts); It; ++It)
	{
		Result.Add(It.Value());
	}
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityMakeshift::GetUniquePlayerId(int32 LocalUserNum) const
{
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityMakeshift::CreateUniquePlayerId(uint8* Bytes, int32 Size)
{
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityMakeshift::CreateUniquePlayerId(const FString& Str)
{
	return MakeShareable(new FUniqueNetIdString(Str));
}

ELoginStatus::Type FOnlineIdentityMakeshift::GetLoginStatus(int32 LocalUserNum) const
{
	TSharedPtr<const FUniqueNetId> UserId = GetUniquePlayerId(LocalUserNum);
	if (UserId.IsValid())
	{
...
ELoginStatus::Type FOnlineIdentityMakeshift::GetLoginStatus(const FUniqueNetId& UserId) const 
{
...
FString FOnlineIdentityMakeshift::GetPlayerNickname(int32 LocalUserNum) const
{
	...
	return TEXT("MakeshiftUser");
}

FString FOnlineIdentityMakeshift::GetPlayerNickname(const FUniqueNetId& UserId) const
{
	return UserId.ToString();
}

FString FOnlineIdentityMakeshift::GetAuthToken(int32 LocalUserNum) const
{
	TSharedPtr<const FUniqueNetId> UserId = GetUniquePlayerId(LocalUserNum);
	if (UserId.IsValid())
	{
...
FOnlineIdentityMakeshift::FOnlineIdentityMakeshift(class FOnlineSubsystemMakeshift* InSubsystem)
{
	// autologin the 0-th player
	Login(0, FOnlineAccountCredentials(TEXT("DummyType"), TEXT("DummyUser"), TEXT("DummyId")) );
}

FOnlineIdentityMakeshift::FOnlineIdentityMakeshift()
{
}
...
void FOnlineIdentityMakeshift::GetUserPrivilege(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, const FOnGetUserPrivilegeCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(UserId, Privilege, (uint32)EPrivilegeResults::NoFailures);
}

FPlatformUserId FOnlineIdentityMakeshift::GetPlatformUserIdFromUniqueNetId(const FUniqueNetId& UniqueNetId)
{
