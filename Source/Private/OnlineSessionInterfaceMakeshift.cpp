...
FOnlineSessionInfoMakeshift::FOnlineSessionInfoMakeshift() :
	HostAddr(NULL),
...
bool FOnlineSessionMakeshift::CreateSession(int32 HostingPlayerNum, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	...
	if (Session == NULL)
	{
		...
		IOnlineIdentityPtr Identity = MakeshiftSubsystem->GetIdentityInterface();
		if (Identity.IsValid())
		{
		...
		if (!Session->OwningUserId.IsValid())
		{
			...
			Session->OwningUserName = FString(TEXT("MakeshiftUser"));
		}
...
bool FOnlineSessionMakeshift::CreateSession(const FUniqueNetId& HostingPlayerId, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	// todo: use proper	HostingPlayerId
	return CreateSession(0, SessionName, NewSessionSettings);
}

bool FOnlineSessionMakeshift::NeedsToAdvertise()
{
...
bool FOnlineSessionMakeshift::NeedsToAdvertise( FNamedOnlineSession& Session )
{
	// In Makeshift, we have to imitate missing online service functionality, so we advertise:
...
uint32 FOnlineSessionMakeshift::UpdateLANStatus()
{
	...
	if ( NeedsToAdvertise() )
	{
		...
		{
			FOnValidQueryPacketDelegate QueryPacketDelegate = FOnValidQueryPacketDelegate::CreateRaw(this, &FOnlineSessionMakeshift::OnValidQueryPacketReceived);
...
bool FOnlineSessionMakeshift::UpdateSession(FName SessionName, FOnlineSessionSettings& UpdatedSessionSettings, bool bShouldRefreshOnlineData)
{
...
bool FOnlineSessionMakeshift::EndSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionMakeshift::DestroySession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionMakeshift::IsPlayerInSession(FName SessionName, const FUniqueNetId& UniqueId)
{
	return IsPlayerInSessionImpl(this, SessionName, UniqueId);
}

bool FOnlineSessionMakeshift::StartMatchmaking(const TArray< TSharedRef<const FUniqueNetId> >& LocalPlayers, FName SessionName, const FOnlineSessionSettings& NewSessionSettings, TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionMakeshift::CancelMatchmaking(int32 SearchingPlayerNum, FName SessionName)
{
...
bool FOnlineSessionMakeshift::CancelMatchmaking(const FUniqueNetId& SearchingPlayerId, FName SessionName)
{
	UE_LOG(LogOnline, Warning, TEXT("CancelMatchmaking is not supported on this platform. Use CancelFindSessions."));
	TriggerOnCancelMatchmakingCompleteDelegates(SessionName, false);
	return false;
...
bool FOnlineSessionMakeshift::FindSessions(int32 SearchingPlayerNum, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionMakeshift::FindSessions(const FUniqueNetId& SearchingPlayerId, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionMakeshift::FindSessionById(const FUniqueNetId& SearchingUserId, const FUniqueNetId& SessionId, const FUniqueNetId& FriendId, const FOnSingleSessionResultCompleteDelegate& CompletionDelegates)
{
...
uint32 FOnlineSessionMakeshift::FindLANSession()
{
	...
	FOnValidResponsePacketDelegate ResponseDelegate = FOnValidResponsePacketDelegate::CreateRaw(this, &FOnlineSessionMakeshift::OnValidResponsePacketReceived);
	FOnSearchingTimeoutDelegate TimeoutDelegate = FOnSearchingTimeoutDelegate::CreateRaw(this, &FOnlineSessionMakeshift::OnLANSearchTimeout);

	FNboSerializeToBufferMakeshift Packet(LAN_BEACON_MAX_PACKET_SIZE);
...
bool FOnlineSessionMakeshift::CancelFindSessions()
{
	uint32 Return = E_FAIL;
...
bool FOnlineSessionMakeshift::JoinSession(int32 PlayerNum, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	uint32 Return = E_FAIL;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
...
bool FOnlineSessionMakeshift::JoinSession(const FUniqueNetId& PlayerId, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	// Assuming player 0 should be OK here
	return JoinSession(0, SessionName, DesiredSession);
}

bool FOnlineSessionMakeshift::FindFriendSession(int32 LocalUserNum, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Makeshift subsystem
...
bool FOnlineSessionMakeshift::FindFriendSession(const FUniqueNetId& LocalUserId, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Makeshift subsystem
	FOnlineSessionSearchResult EmptySearchResult;
...
bool FOnlineSessionMakeshift::SendSessionInviteToFriend(int32 LocalUserNum, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Makeshift subsystem
	return false;
};

bool FOnlineSessionMakeshift::SendSessionInviteToFriend(const FUniqueNetId& LocalUserId, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Makeshift subsystem
	return false;
}

bool FOnlineSessionMakeshift::SendSessionInviteToFriends(int32 LocalUserNum, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Makeshift subsystem
	return false;
};

bool FOnlineSessionMakeshift::SendSessionInviteToFriends(const FUniqueNetId& LocalUserId, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Makeshift subsystem
	return false;
}

uint32 FOnlineSessionMakeshift::JoinLANSession(int32 PlayerNum, FNamedOnlineSession* Session, const FOnlineSession* SearchSession)
{
	...
	if (Session->SessionInfo.IsValid() && SearchSession != nullptr && SearchSession->SessionInfo.IsValid())
	{
		...
		const FOnlineSessionInfoMakeshift* SearchSessionInfo = (const FOnlineSessionInfoMakeshift*)SearchSession->SessionInfo.Get();
		FOnlineSessionInfoMakeshift* SessionInfo = (FOnlineSessionInfoMakeshift*)Session->SessionInfo.Get();
...
bool FOnlineSessionMakeshift::PingSearchResults(const FOnlineSessionSearchResult& SearchResult)
{
	return false;
}

/** Get a resolved connection string from a session info */
static bool GetConnectStringFromSessionInfo(TSharedPtr<FOnlineSessionInfoMakeshift>& SessionInfo, FString& ConnectInfo, int32 PortOverride=0)
{
	bool bSuccess = false;
...
bool FOnlineSessionMakeshift::GetResolvedConnectString(FName SessionName, FString& ConnectInfo)
{
	bool bSuccess = false;
	// Find the session
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session != NULL)
	{
		TSharedPtr<FOnlineSessionInfoMakeshift> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoMakeshift>(Session->SessionInfo);
...
bool FOnlineSessionMakeshift::GetResolvedConnectString(const class FOnlineSessionSearchResult& SearchResult, FName PortType, FString& ConnectInfo)
{
	bool bSuccess = false;
	if (SearchResult.Session.SessionInfo.IsValid())
	{
		TSharedPtr<FOnlineSessionInfoMakeshift> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoMakeshift>(SearchResult.Session.SessionInfo);
...
FOnlineSessionSettings* FOnlineSessionMakeshift::GetSessionSettings(FName SessionName) 
{
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
	{
...
void FOnlineSessionMakeshift::RegisterLocalPlayers(FNamedOnlineSession* Session)
{
	if (!MakeshiftSubsystem->IsDedicated())
	{
		IOnlineVoicePtr VoiceInt = MakeshiftSubsystem->GetVoiceInterface();
		if (VoiceInt.IsValid())
		{
...
void FOnlineSessionMakeshift::RegisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = MakeshiftSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
		if (!MakeshiftSubsystem->IsLocalPlayer(PlayerId))
		{
...
void FOnlineSessionMakeshift::UnregisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = MakeshiftSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
...
bool FOnlineSessionMakeshift::RegisterPlayer(FName SessionName, const FUniqueNetId& PlayerId, bool bWasInvited)
{
...
bool FOnlineSessionMakeshift::RegisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players, bool bWasInvited)
{
	bool bSuccess = false;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
...
bool FOnlineSessionMakeshift::UnregisterPlayer(FName SessionName, const FUniqueNetId& PlayerId)
{
	TArray< TSharedRef<const FUniqueNetId> > Players;
	Players.Add(MakeShareable(new FUniqueNetIdString(PlayerId)));
...
bool FOnlineSessionMakeshift::UnregisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players)
{
	bool bSuccess = true;
...
void FOnlineSessionMakeshift::Tick(float DeltaTime)
{
	SCOPE_CYCLE_COUNTER(STAT_Session_Interface);
	TickLanTasks(DeltaTime);
}

void FOnlineSessionMakeshift::TickLanTasks(float DeltaTime)
{
	LANSessionManager.Tick(DeltaTime);
}

void FOnlineSessionMakeshift::AppendSessionToPacket(FNboSerializeToBufferMakeshift& Packet, FOnlineSession* Session)
{
	...
	SetPortFromNetDriver(*MakeshiftSubsystem, Session->SessionInfo);

	// Write host info (host addr, session id, and key)
	Packet << *StaticCastSharedPtr<FOnlineSessionInfoMakeshift>(Session->SessionInfo);

	// Now append per game settings
	AppendSessionSettingsToPacket(Packet, &Session->SessionSettings);
}

void FOnlineSessionMakeshift::AppendSessionSettingsToPacket(FNboSerializeToBufferMakeshift& Packet, FOnlineSessionSettings* SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionMakeshift::OnValidQueryPacketReceived(uint8* PacketData, int32 PacketLength, uint64 ClientNonce)
{
...
void FOnlineSessionMakeshift::ReadSessionFromPacket(FNboSerializeFromBufferMakeshift& Packet, FOnlineSession* Session)
{
#if DEBUG_LAN_BEACON
	...
	FOnlineSessionInfoMakeshift* MakeshiftSessionInfo = new FOnlineSessionInfoMakeshift();
	MakeshiftSessionInfo->HostAddr = ISocketSubsystem::Get()->CreateInternetAddr();
	Packet >> *MakeshiftSessionInfo;
	Session->SessionInfo = MakeShareable(MakeshiftSessionInfo); 

	// Read any per object data using the server object
	ReadSettingsFromPacket(Packet, Session->SessionSettings);
}

void FOnlineSessionMakeshift::ReadSettingsFromPacket(FNboSerializeFromBufferMakeshift& Packet, FOnlineSessionSettings& SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionMakeshift::OnValidResponsePacketReceived(uint8* PacketData, int32 PacketLength)
{
	...
	if (CurrentSessionSearch.IsValid())
	{
		...
		FNboSerializeFromBufferMakeshift Packet(PacketData, PacketLength);
...
uint32 FOnlineSessionMakeshift::FinalizeLANSearch()
{
	if (LANSessionManager.GetBeaconState() == ELanBeaconState::Searching)
	{
		LANSessionManager.StopLANSession();
...
void FOnlineSessionMakeshift::OnLANSearchTimeout()
{
	FinalizeLANSearch();
...
int32 FOnlineSessionMakeshift::GetNumSessions()
{
	FScopeLock ScopeLock(&SessionLock);
	return Sessions.Num();
}

void FOnlineSessionMakeshift::DumpSessionState()
{
	FScopeLock ScopeLock(&SessionLock);
...
void FOnlineSessionMakeshift::RegisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnRegisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, EOnJoinSessionCompleteResult::Success);
}

void FOnlineSessionMakeshift::UnregisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnUnregisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, true);
}

void FOnlineSessionMakeshift::SetPortFromNetDriver(const FOnlineSubsystemMakeshift& Subsystem, const TSharedPtr<FOnlineSessionInfo>& SessionInfo)
{
	auto NetDriverPort = GetPortFromNetDriver(Subsystem.GetInstanceName());
	auto SessionInfoMakeshift = StaticCastSharedPtr<FOnlineSessionInfoMakeshift>(SessionInfo);
	if (SessionInfoMakeshift.IsValid() && SessionInfoMakeshift->HostAddr.IsValid())
	{
		SessionInfoMakeshift->HostAddr->SetPort(NetDriverPort);
	}
}

bool FOnlineSessionMakeshift::IsHost(const FNamedOnlineSession& Session) const
{
	if (MakeshiftSubsystem->IsDedicated())
	{
		return true;
	...
	IOnlineIdentityPtr IdentityInt = MakeshiftSubsystem->GetIdentityInterface(); 
